...
describe('server QR PNG renderer', () => {
  it('renders a real PNG using the current QR geometry', () => {
    const out = renderPairTokenPng('https://captcha-dev-jw.argus.pw', '0DJ06tuZ5eEzdLcJJRWWwg');
  ...
  it('keeps the redeem URL inside the PNG image, not as plaintext transport bytes', () => {
    const token = '0DJ06tuZ5eEzdLcJJRWWwg';
    const out = renderPairTokenPng('https://captcha-dev-jw.argus.pw', token);
  ...
  it('accepts a phone-entry suffix without mutating the public origin', () => {
    const out = renderPairTokenPng(
  ...
  it('renders an animated frame sweep for the same token URL', () => {
    const token = '0DJ06tuZ5eEzdLcJJRWWwg';
    const out = renderPairTokenPngFrames('https://captcha-dev-jw.argus.pw', token);
