...
type Phase = 'redeeming' | 'returning' | 'error';
...
export function HostedCallback() {
      ...
      if (alive) {
        if (result.verdict === 'passed') {
          window.sessionStorage.setItem(
            'argus-hosted-entry',
            JSON.stringify({ code, reason: result.reason, receivedAt: Date.now() })
          );
          setPhase('returning');
          window.location.replace('/?hosted=passed');
        } else {
          window.location.replace(
            `/?hosted=failed&reason=${encodeURIComponent(result.reason ?? '')}`
          );
        }
      }
    })().catch((e) => {
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-sm flex-col gap-8 px-6 py-10">
      ...
      {(phase === 'redeeming' || phase === 'returning') && (
        <div className="flex flex-1 flex-col items-center justify-center gap-6 text-center">
          ...
          <div className="mt-1 flex items-center justify-center gap-2 text-xs text-muted">
            ...
            <span className="pulse-fade">
              {phase === 'returning' ? 'returning to entry' : 'redeeming one-time code'}
            </span>
          </div>
        </div>
      )}
