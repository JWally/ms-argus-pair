export type CpiScope = 'integrity' | 'stepup';
...
const SCOPED_CPI_RE = /^(argus_cpi_(?:test|live)_[A-Za-z0-9]{10,40})(?:\.(stepup))?$/;
...
export function parseScopedCpi(value: unknown): ScopedCpi | null {
  ...
  const scope: CpiScope = match[2] === 'stepup' ? 'stepup' : 'integrity';
